class pt14 : public timed_petri_net
{
  ...
  {
    u8 _N__4338[] = { (BYTE)1, bt };
    return CAN_MESSAGE(721, IntInf.fromInt(0), byte2(_N__4338));
  }
    ...
    if (lock(pl_UDP_AND_CAN_SEND | pl_UDP_AND_CAN_SEND | pl_BinTimer | pl_BatLevel | pl_BinTimer | pl_BUTTONS))
    {
      if (BatLevel_flag)
      {
        if (BinTimer.have_tokens())
        {
          ...
          UDP_AND_CAN_SEND.add(tuple3(336, get_time(), (_N__4336)));
          UDP_AND_CAN_SEND.add(tuple3(721, get_time(), (_N__4337)));
