...
private:
	...
	typedef enum
	{
		...
		tr_UnnamedTransition9 = 0x400,
		tr_UnnamedTransition11 = 0x800,
		tr_BUTTONS = 0x1000,
		tr_UnnamedTransition12 = 0x2000,
		tr_UnnamedTransition13 = 0x4000,
		tr_UnnamedTransition16 = 0x8000,
...
UnnamedTransition17
	...
	{
		bool res = false;
		if (lock(pl_SYNC | pl_SyncFreq, tr_UnnamedTransition8))
		{
			if (SyncFreq)
			{
				;
	...
	{
		bool res = false;
		if (lock(pl_SEND_SENS | pl_UDP_SEND | pl_UDP_OUT | pl_COUNTER | pl_UDP_IN, tr_GroupTransition1021))
		{
			if (UDP_IN_flag)
			{
				const CAN_UDP_MESSAGE &m = UDP_IN;
			...
			unlock(pl_SEND_SENS | pl_UDP_SEND | pl_UDP_OUT | pl_COUNTER | pl_UDP_IN);
		}
		return res;
	}
	...
	{
		bool res = false;
		if (lock(pl_SEND_SENS | pl_UDP_SEND | pl_UDP_OUT | pl_COUNTER | pl_DELAY, tr_UnnamedTransition11))
		{
			;
			...
			SEND_SENS(1);
			res = true;;
			unlock(pl_SEND_SENS | pl_UDP_SEND | pl_UDP_OUT | pl_COUNTER | pl_DELAY);
		}
		return res;
	}
	...
	bool UnnamedTransition16()
	{
		...
		if (lock(pl_CAN_OUT | pl_CAN, tr_UnnamedTransition16))
		{
			...
			if (CAN.have_tokens())
			{
				...
				CAN.get(cm);
				;
				bool in = CAN_OUT.exec_ref_ret(cm);
	...
	bool UnnamedTransition17(const CAN_MESSAGE &cm)
	{
		bool res = false;
