...
private:
  ...
  bool UnnamedTransition8()
  {
    ...
    if (lock(pl_COUNTER | pl_UDP_OUT | pl_CAN | pl_CAN_OUT | pl_BatLevel | pl_BinTimer | pl_BUTTONS, tr_UnnamedTransition8))
    {
      if (BatLevel_flag)
      {
        ...
        UDP_AND_CAN_SEND(CAN_MESSAGE(336, time64(), (_N__4351)));
        UDP_AND_CAN_SEND(CAN_MESSAGE(721, time64(), (_N__4352)));
        res = true;
      };
  ...
  {
    u8 _N__4353[] = { (BYTE)1, bt };
    return CAN_MESSAGE(721, time(), byte2(_N__4353));
  }
