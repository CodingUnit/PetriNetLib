class pt14 : public timed_petri_net
{
	...
	typedef enum
	{
		...
		tr_GroupTransition1287 = 0x4,
		tr_UnnamedTransition6 = 0x8,
		tr_UnnamedTransition8 = 0x10,
		tr_UnnamedTransition9 = 0x20,
		tr_DPS = 0x40,
		tr_UnnamedTransition0 = 0x80,
		tr_UnnamedTransition10 = 0x100,
		tr_UnnamedTransition11 = 0x200,
		tr_UnnamedTransition1 = 0x400,
		tr_UnnamedTransition2 = 0x800,
		tr_UnnamedTransition3 = 0x1000,
		tr_UnnamedTransition4 = 0x2000,
		tr_BUTTONS = 0x4000,
		tr_UnnamedTransition5 = 0x8000,
		tr_UnnamedTransition13 = 0x10000,
		tr_UnnamedTransition14 = 0x20000,
		tr_UnnamedTransition15 = 0x40000,
		tr_UnnamedTransition16 = 0x80000,
		tr_UnnamedTransition17 = 0x100000,
		tr_UnnamedTransition18 = 0x200000
	} ttran;
...
public:
	...
	{
		if (lock(pl_UDP_SEND | pl_GPS, tr_UnnamedTransition6))
		{
			if (GPS_flag)
			{
				bytesn s = GPS;
	...
	{
		if (lock(pl_UDP_SEND | pl_UDP_IN, tr_GroupTransition1287))
		{
			if (UDP_IN_flag)
			{
				...
				if (is_control(m))
				{
					...
					tran_ena(tr_UnnamedTransition14, 0);
					return true;
				}
			};
	...
	{
		if (lock(pl_CAN | pl_UDP_IN, tr_GroupTransition1287))
		{
			if (UDP_IN_flag)
			{
				CAN_UDP_MESSAGE m = UDP_IN;
	...
	{
		if (lock(pl_UDP_SEND | pl_CAN_IN, tr_UnnamedTransition9))
		{
			if (CAN_IN_flag)
			{
	...
	bool DPS()
	{
		if (lock(pl_SYNC_BUF | pl_UDP_SEND | pl_SYNC | pl_DELAY | pl_SSI | pl_SYNC | pl_DPS, tr_DPS))
		{
			; ;
			bytes8 ssi = SSI.peek();
			int n = SYNC;
			...
			tran_ena(tr_UnnamedTransition10 | tr_UnnamedTransition14 | tr_UnnamedTransition11, 0);
			return true;;
	...
	{
		if (lock(pl_DEBUG_TIMER | pl_UDP_SEND | pl_DEBUG_TIMER | pl_SEND_DEBUG, tr_UnnamedTransition0))
		{
	...
	{
		if (lock(pl_DELAY | pl_SYNC_BUF, tr_UnnamedTransition10))
		{
			if (SYNC_BUF_flag)
			{
				SYNC_BUF_flag = false;
	...
	{
		if (lock(pl_UDP_SEND | pl_SEND_SENS | pl_SSI | pl_SYNC | pl_DPS, tr_UnnamedTransition11))
		{
			if (SEND_SENS_flag)
			{
				bytes8 ssi = SSI.peek();
	...
	{
		if (lock(pl_UDP_AND_CAN_SEND | pl_UDP_AND_CAN_SEND | pl_BinTimer | pl_BatLevel | pl_BinTimer | pl_BUTTONS, tr_UnnamedTransition1))
		{
			if (BatLevel_flag)
			{
	...
	{
		if (lock(pl_SyncFreq | pl_SYNC | pl_SyncFreq, tr_UnnamedTransition2))
		{
	...
	{
		if (lock(pl_UDP_AND_CAN_SEND | pl_TempTimer | pl_TempTimer | pl_TEMP, tr_UnnamedTransition3))
		{
	...
	{
		if (lock(pl_SEND_SENS | pl_UDP_IN, tr_GroupTransition1287))
		{
			if (UDP_IN_flag)
			{
				CAN_UDP_MESSAGE m = UDP_IN;
				if (is_sens(m))
				{
					UDP_IN_flag = false;
					;
					SEND_SENS = token_iter_utils.repeat('u', 1);
					tran_ena(tr_UnnamedTransition11, 0);
					return true;
				}
			};
	...
	{
		if (lock(pl_SEND_SENS | pl_DELAY | pl_DELAY, tr_UnnamedTransition4))
		{
	...
	bool BUTTONS()
	{
		if (lock(pl_UDP_AND_CAN_SEND | pl_BUTTONS, tr_BUTTONS))
		{
			u8 bt = BUTTONS;
			;
			UDP_AND_CAN_SEND.add(bt2can_message(bt));
			tran_ena(tr_UnnamedTransition15, 0);
			return true;;
	...
	{
		if (lock(pl_ADC_SUM | pl_TIMER | pl_ADC_SUM | pl_TIMER | pl_ADC, tr_UnnamedTransition5))
		{
			if (ADC_SUM_flag)
			{
	...
	{
		if (lock(pl_COUNTER | pl_UDP_OUT | pl_COUNTER | pl_UDP_SEND, tr_UnnamedTransition14))
		{
			int n = COUNTER;
	...
	{
		if (lock(pl_UDP_SEND | pl_CAN | pl_UDP_AND_CAN_SEND, tr_UnnamedTransition15))
		{
	...
	{
		if (lock(pl_CAN_OUT, tr_UnnamedTransition18))
		{
			if (CAN_OUT_flag)
			{
				if (res)
