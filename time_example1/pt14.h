...
public:
  ...
  void UDP_SEND(const CAN_MESSAGE &param)
  {
    bool res = false;
    if (lock(pl_COUNTER | pl_UDP_OUT | pl_COUNTER | pl_UDP_SEND, tr_UnnamedTransition14))
    {
      ...
      COUNTER = n + 1;
      tran_ena(0, tr_UnnamedTransition14);
      res = true;;
  ...
  {
    bool res = false;
    if (lock(pl_DELAY | pl_SYNC_BUF, tr_UnnamedTransition5))
    {
      ;
      DELAY = 1;
      DELAY_time = time();
      tran_ena(0, tr_UnnamedTransition5);
      res = true;;
  ...
  void UDP_AND_CAN_SEND(const CAN_MESSAGE &param)
  {
    bool res = false;
    if (lock(pl_UDP_SEND | pl_UDP_OUT | pl_COUNTER | pl_UDP_AND_CAN_SEND, tr_UnnamedTransition15))
    {
      ;
      UDP_SEND(cm);
      CAN.add(cm);
      tran_ena(0, tr_UnnamedTransition15);
      UnnamedTransition16();
      res = true;;
      unlock(pl_UDP_SEND | pl_UDP_OUT | pl_COUNTER | pl_UDP_AND_CAN_SEND);
    }
    return res;
  }
